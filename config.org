...
#+begin_src emacs-lisp
  ...
  (use-package magit
    ...
    (("C-x g g" . magit-status)
     ...
     ("C-x g F" . magit-find-file-other-window)
    ...
    :config
    ;; ensure magit does not split window more than once
    ...
    ;; Function to diff against upstream merge-base
    (transient-define-suffix magit-diff-base (args)
      "Show diff between merge-base of upstream and HEAD.
  Equivalent to: git diff $(git merge-base @{u} HEAD) HEAD"
      :description "diff base"
      ...
      (let* ((upstream (magit-get-upstream-branch))
        ...
        (if merge-base
            (magit-diff-range (concat merge-base "..HEAD") args)
          (user-error "No upstream branch configured or merge-base not found"))))

    ;; Add the function to magit-diff transient menu
    (transient-append-suffix 'magit-diff "d"
      '("b" "diff base" magit-diff-base))

    ;; Rebase onto upstream merge-base
    (transient-define-suffix magit-rebase-base (args)
      "Rebase onto merge-base of upstream and HEAD.
  Equivalent to: git rebase $(git merge-base @{u} HEAD)"
      ...
      (let* ((upstream (magit-get-upstream-branch))
        ...
        (if merge-base
            (progn
              (message "Rebasing...")
              (magit-git-rebase merge-base args)
              (message "Rebasing...done"))
          (user-error "No upstream branch configured or merge-base not found"))))

    (transient-append-suffix 'magit-rebase "e"
      '("b" "merge-base" magit-rebase-base))
    ...
    (defun my-magit-read-worktree-directory-in-repo (prompt branch)
      "Read worktree directory defaulting to <repo-root>/worktrees/BRANCH."
    ...
    (setq magit-read-worktree-directory-function
          #'my-magit-read-worktree-directory-in-repo)
...
#+begin_src emacs-lisp
  (use-package forge
    ...
    :config
    (defun my/forge-insert-authored-pullreqs ()
      "Insert pull requests authored by me into the magit status buffer."
      (forge-insert-topics 'my-authored-pullreqs "My pull requests"
        (lambda (repo)
          (and-let* ((me (ghub--username repo)))
            (forge--topics-spec :type 'pullreq
                                :active nil
                                :author me
                                :state 'open
    ...
    (defun my/forge-insert-assigned-pullreqs ()
      ...
      (forge-insert-topics 'assigned-pullreqs "Assigned pull requests"
        (lambda (repo)
          (and-let* ((me (ghub--username repo)))
            (forge--topics-spec :type 'pullreq :assignee me :active nil :state 'open :order 'recently-updated )
            ))))
    (defun my/forge-insert-review-requests ()
      ...
      (forge-insert-topics 'reviewer-pullreqs "Review requests"
        (lambda (repo)
          (and-let* ((me (ghub--username repo)))
            (forge--topics-spec :type 'pullreq :active nil :state 'open :order 'recently-updated :reviewer me)))))
  ...
  `assignee' slots."
      ...
      (forge-insert-topics 'assigned-issues "Assigned issues"
        ...
        ;;; ... to here instead.  For example ----,
          (and-let* ((me (ghub--username repo)))
            (forge--topics-spec :type 'issue :assignee me :active nil :state 'open)))))

  (remove-hook 'magit-status-sections-hook #'forge-insert-issues)
  (remove-hook 'magit-status-sections-hook #'forge-insert-pullreqs)
  (remove-hook 'magit-status-sections-hook #'forge-insert-discussions)

  (add-hook 'magit-status-sections-hook #'my/forge-insert-authored-pullreqs t)
  (add-hook 'magit-status-sections-hook #'my/forge-insert-assigned-pullreqs t)
  (add-hook 'magit-status-sections-hook #'my/forge-insert-review-requests t)
  (add-hook 'magit-status-sections-hook #'my/forge-insert-assigned-issues t)
...
#+begin_src emacs-lisp
  (use-package projectile
    :custom
    (projectile-switch-project-action 'projectile-run-vterm)
    :config
    (defun my/project-switch-to-recent ()
      "Switch to the most recently used buffer in the project.
Falls back to the most recent file from `recentf-list',
then to `project-find-file'."
      (interactive)
      (let* ((proj (project-current t))
             (root (expand-file-name (project-root proj)))
             (buf  (car (project-buffers proj))))
        (cond
         (buf (switch-to-buffer buf))
         ((and (bound-and-true-p recentf-mode)
               (seq-find (lambda (f) (file-in-directory-p f root)) recentf-list))
          (find-file (seq-find (lambda (f) (file-in-directory-p f root)) recentf-list)))
         (t (project-find-file)))))
    (setq project-switch-commands #'my/project-switch-to-recent)
    (defun my/project-prompt-prefer-previous (orig &rest args)
      "Preselect the previous project in `project-prompt-project-dir',
mimicking `switch-to-buffer'. Relies on `project--list' being MRU-ordered."
      (project--ensure-read-project-list)
      (let ((prev (car-safe (cadr project--list))))
        (if (null prev)
            (apply orig args)
          (cl-letf* ((orig-cr (symbol-function 'completing-read))
                     ((symbol-function 'completing-read)
                      (lambda (_prompt coll &optional pred req init hist _def inherit)
                        (funcall orig-cr
                                 (format "Select project (default %s): "
                                         (abbreviate-file-name prev))
                                 coll pred req init hist prev inherit))))
            (apply orig args)))))
    (advice-add 'project-prompt-project-dir :around #'my/project-prompt-prefer-previous)
    (with-eval-after-load 'consult
      (plist-put consult-source-project-root
                 :action (lambda (root)
                           (let ((default-directory root))
                             (my/project-switch-to-recent)))))
    (projectile-mode 1)
    (setq projectile-completion-system 'auto)
    (setq projectile-enable-caching t)
    ;; Skip TRAMP paths during project root detection to avoid slow
    ;; consult-buffer when vterm buffers are SSH'd into remote hosts
    (setq projectile-ignored-project-function #'file-remote-p)
    (setq compilation-scroll-output t)
    (defun copy-project-relative-path ()
      "Copy current file path relative to Projectile project root."
      (interactive)
      (let* ((file (cond
        ...
        (let* ((root (or (ignore-errors (projectile-project-root))
               ...
               (rel  (file-relative-name (expand-file-name file) (expand-file-name root))))
          (kill-new rel)
...
#+begin_src emacs-lisp
  (use-package vterm
    ...
    (add-hook 'vterm-mode-hook
              (lambda ()
                (unless (string-match-p "claude" (buffer-name))
                  (with-editor-export-editor))))
    ;; fix cursor disappearing in copy mode when using claude-code
    (add-hook 'vterm-copy-mode-hook
              (lambda ()
                (when (bound-and-true-p claude-code-mode)
                  (setq-local cursor-type (if vterm-copy-mode t t)))))
    :hook
    (vterm-mode . (lambda ()
                    (unless (string-match-p "claude" (buffer-name))
                      (xterm-mouse-mode 1))))
...
#+begin_src emacs-lisp
  (use-package tree-sitter-langs
    ...
    (when (hash-table-p tree-sitter-major-mode-language-table)
      (puthash 'forge-post-mode 'markdown tree-sitter-major-mode-language-table))
    :hook
...
#+begin_src emacs-lisp
  ...
  (use-package vertico-posframe
    ...
    (setq vertico-posframe-default-width (round (* 0.6 (frame-width))))
    (defun vertico-posframe-cycle-width ()
      (interactive)
      (if (eq vertico-posframe-width vertico-posframe-max-width)
          (setq vertico-posframe-width (round (* 0.6 (frame-width))))
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    ("M-g r s" . consult-register-store)
    ("M-g r l" . consult-register-load)
    ("M-g r r" . consult-register)
    ("M-g m" . consult-mark)
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (with-eval-after-load 'embark
      (defun my/embark-projectile-vterm (project-root)
        "Open vterm in PROJECT-ROOT (a Projectile project directory string)."
        (interactive "DProject root: ")
        (let ((default-directory (file-name-as-directory project-root)))
          (vterm)))
      )
    (define-key embark-file-map (kbd "v") #'my/embark-projectile-vterm))
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    (:map lsp-command-map ("e" . consult-lsp-file-diagnostics))
    (:map lsp-command-map ("E" . consult-lsp-diagnostics-repo))
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    ;;(add-hook 'corfu-mode-hook 'my-trigger-hook)
...
#+begin_src emacs-lisp
  (use-package copilot
    ...
    :straight (:host github :repo "copilot-emacs/copilot.el"
                     :branch "main")
    :custom
    (copilot-node-executable "/home/eduplens/.local/bin/node")
...
** claude
#+begin_src emacs-lisp
  (use-package web-server)  ;; required by claude-code-ide MCP server
  (use-package websocket)   ;; required by claude-code-ide MCP server
  (use-package claude-code-ide
    :straight (:host github :repo "manzaltu/claude-code-ide.el")
    :bind
    ("C-c C-'" . claude-code-ide-menu) ; Set your favorite keybinding
    ("M-'" . claude-code-ide-continue) ; Set your favorite keybinding
    ("C-g" . keyboard-quit) ; Set your favorite keybinding
    :config
    (claude-code-ide-emacs-tools-setup)
    ;; Prevent vterm from renaming claude-code-ide buffers (gh#103)
    (advice-add 'claude-code-ide--configure-vterm-buffer :after
                (lambda () (setq-local vterm-buffer-name-string nil)))
    ;; C-g sends Escape to CLI (like claude-code.el does)
    (add-hook 'vterm-mode-hook
              (lambda ()
                (when (string-match-p "claude-code" (buffer-name))
                  (let ((map (make-sparse-keymap)))
                    ...
                    (define-key map (kbd "C-g")
                                (lambda () (interactive) (vterm-send-key "\C-[")))
                    (use-local-map map)))))
    :custom
    (claude-code-ide-window-width (lambda (_win) (/ (frame-width) 2)))
    (claude-code-ide-prevent-reflow-glitch nil)
    (claude-code-ide-no-flicker nil) ;; does not allow to scroll back
    )
  #+end_src
* email
* org-mode
...
#+begin_src emacs-lisp
  (use-package eaf
    ...
    (eaf-proxy-type "http")
    (eaf-proxy-host "proxy-mu.intel.com")
    (eaf-proxy-port "911")
    :config
    (require 'eaf-browser)
    (require 'eaf-system-monitor)
    (setq eaf-python-command "~/.virtualenvs/openvino/bin/python3")
    )
#+end_src
